...
#define ES8323_HP_PIN  RK30_PIN4_PC5	// need modify.
#endif

#undef SPK_CTL

#define SPK_CON 		RK30_PIN0_PC7 //RK30_PIN4_PC5

static int playing = 0;
static int hp_on = 0;
static int hdmi_on = 0;

//#define SPK_CTL             RK29_PIN6_PB6
//#define EAR_CON_PIN             RK29_PIN6_PB5
#undef EAR_CON_PIN

static char es8323_spk_gain=0x1f;
static char es8323_ear_gain=0x1f;


static struct snd_soc_codec *es8323_codec;
static void update_volume(int volume)
{
	int old = snd_soc_read(es8323_codec, ES8323_LOUT2_VOL);
	int step = volume > old ? 1 : -1;
	while(volume != old) {
		old += step;
		snd_soc_write(es8323_codec, ES8323_LOUT2_VOL, old);
		snd_soc_write(es8323_codec, ES8323_ROUT2_VOL, old);
	}
}
static void update_spk_con(void)
{
	int i;
	int value = playing && !hp_on && !hdmi_on;
	gpio_set_value(SPK_CON, value);
	if(es8323_codec && playing) {
		if(value) {
			update_volume(es8323_spk_gain);
/*
			snd_soc_write(es8323_codec,0x1E,0x02); //-12DB,Bass@600Hz,Fs=44100
			snd_soc_write(es8323_codec,0x1F,0x32);
			snd_soc_write(es8323_codec,0x20,0x59);
			snd_soc_write(es8323_codec,0x21,0x3b);
			snd_soc_write(es8323_codec,0x22,0x20);
			snd_soc_write(es8323_codec,0x23,0x3a);
			snd_soc_write(es8323_codec,0x24,0xd8);
			snd_soc_write(es8323_codec,0x25,0xf8);
*/
			snd_soc_write(es8323_codec,0x1E,0x01); //-12DB,Bass@400Hz,Fs=44100
			snd_soc_write(es8323_codec,0x1F,0x22);
			snd_soc_write(es8323_codec,0x20,0xE5);
			snd_soc_write(es8323_codec,0x21,0xDF);
			snd_soc_write(es8323_codec,0x22,0x20);
			snd_soc_write(es8323_codec,0x23,0x4B);
			snd_soc_write(es8323_codec,0x24,0xAB);
			snd_soc_write(es8323_codec,0x25,0x8F);

		}
		else {
			update_volume(es8323_ear_gain);
			snd_soc_write(es8323_codec, 0x1E,0x01);    //for 47uF capacitors ,6db Bass@100Hz,Fs=44100
			snd_soc_write(es8323_codec, 0x1F,0x64);
			snd_soc_write(es8323_codec, 0x20,0x29);
			snd_soc_write(es8323_codec, 0x21,0x9D);
			snd_soc_write(es8323_codec, 0x22,0x20);
			snd_soc_write(es8323_codec, 0x23,0x5B);
			snd_soc_write(es8323_codec, 0x24,0xE7);
			snd_soc_write(es8323_codec, 0x25,0x9D);
		}
	}
	//printk("spk con %d, playing %d, hdmi %d, hp %d\n", value, playing, hdmi_on, hp_on);
}

static void reset_spk_con(void)
{
	playing = 0;
	update_spk_con();
}
...
{
	int hp_now = !gpio_get_value(ES8323_HP_PIN);
	if(hp_now != hp_on) {
		hp_on = hp_now;
		update_spk_con();
	}
...
static int es8323_probe(struct snd_soc_codec *codec)
{
  ...
  //es8323_set_bias_level(codec, SND_SOC_BIAS_STANDBY);
	...
	//-----------------------------------------------------------------------------------------------------------------
	snd_soc_write(codec, ES8323_ADCCONTROL1, env_get_u32("es8323_mic_gain", 0x88));     // MIC PGA gain: +24dB
	...
	snd_soc_write(codec, ES8323_ADCCONTROL10, 0xe2);    // ALC stereo, Max gain(17.5dB), Min gain(0dB),updated by david-everest,5-25
	...
	snd_soc_write(codec, ES8323_ADCCONTROL14, 0x93);    // NGTH(XXX), NGG(mute ADC output), NGAT(enable)

	//----------------------------------------------------------------------------------------------------------------
	snd_soc_write(codec, ES8323_LDAC_VOL, 0x04);    // left DAC volume
	snd_soc_write(codec, ES8323_RDAC_VOL, 0x04);    // right DAC volume

	snd_soc_write(codec, 0x1E,0x01);    //for 47uF capacitors ,6db Bass@100Hz,Fs=44100
	snd_soc_write(codec, 0x1F,0x64);
	snd_soc_write(codec, 0x20,0x29);
	snd_soc_write(codec, 0x21,0x9D);
	snd_soc_write(codec, 0x22,0x20);
	snd_soc_write(codec, 0x23,0x5B);
	snd_soc_write(codec, 0x24,0xE7);
	snd_soc_write(codec, 0x25,0x9D);
	...
	snd_soc_write(codec, ES8323_CHIPPOWER   , 0x00);
	snd_soc_write(codec, ES8323_DACPOWER, 0x0c);
	es8323_spk_gain=env_get_u32("es8323_spk_gain", 0x1f);
	es8323_ear_gain=env_get_u32("es8323_ear_gain", 0x1f);
	update_volume(es8323_spk_gain);
#endif
