...
static struct wake_lock * mt5931_lock;
void wake_lock_init(struct wake_lock *lock, int type, const char *name)
{
	...
	list_add(&lock->link, &inactive_locks);
	spin_unlock_irqrestore(&list_lock, irqflags);
	//work around for mt5931 timer
	if(!strcmp("WLAN Timer", name)) {
		printk("## request wlan timer\n");
		if(mt5931_lock) {
			printk("## unreleased wlan timer %p\n", mt5931_lock->name);
			mt5931_lock->name = name;
			wake_lock_destroy(mt5931_lock);
		}
		mt5931_lock = lock;
	}
}
