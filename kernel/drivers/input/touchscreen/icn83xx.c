...
static ssize_t icn83xx_ctpfwupdate_show(struct device *dev,struct device_attribute *attr, char *buf)  
{  	 
		if(ctpfwinfo==0)
		{
			sprintf(buf, "device id: 0x%x<br><br>fw version: 0x%x\n",icn83xx_readDevieID(),icn83xx_readVersion());
		}else if(ctpfwinfo==1)
		{	
...
static ssize_t icn83xx_ctpfwupdate_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{  	
	 char fwpath[128];
	 ...
	 if(buf==NULL){
		...
		bforce=true;
	 	printk("[%d] icn83xx use force update fw.\n",'\n');
		 ...
		 disable_irq(TS_INT);
		 ctpfwinfo=2;
		 fwversion=icn83xx_read_fw_Ver(fwpath);
		 if(fwversion < 0)
		 {
			strcpy(fw_status,"***err: read fw file fail***\n");
			ctpfwinfo=0;
		 	return count;
		 }
		 ...
		 if(icn83xx_readVersion()>=fwversion  && !bforce)
		 {
		 ...
		 /*
		 if(icn83xx_readDevieID()!=deviceid() && !bforce)
		 {
...
static DEVICE_ATTR(ctpfwupdate, S_IRUGO|S_IWUSR, icn83xx_ctpfwupdate_show, icn83xx_ctpfwupdate_store); 
...
static int icn83xx_create_sysfs(struct i2c_client *client)
{
	...
	icn83xx_ts=i2c_get_clientdata(client);
	// /sys/devices/platform/rk30_i2c.2/i2c-2/2-0030/update 
	// struct device *dev = &(client->dev);
	
	// /sys/devices/virtual/input/input3/update
	struct device *dev = &(icn83xx_ts->input_dev->dev);

	err = device_create_file(dev, &dev_attr_ctpfwupdate);
...
//	pr_info("==icn83xx_read_data ==\n");
	...
	switch(data->point_num)
	{
		case 5:
			...
			{
				swap(data->point_info[4].u16PosX, data->point_info[4].u16PosY);
			}
			if(1 == revert_x_flag)
			{
		...
		case 4:
			...
			if(exchange_x_y_flag) 
			{
				swap(data->point_info[3].u16PosX, data->point_info[3].u16PosY);
			}
			if(1 == revert_x_flag)
			{
		...
		case 3:
			...
			if(exchange_x_y_flag) 
			{
				swap(data->point_info[2].u16PosX, data->point_info[2].u16PosY);
			}
			if(1 == revert_x_flag)
			{
		...
		case 2:
			...
			if(exchange_x_y_flag) 
			{
				swap(data->point_info[1].u16PosX, data->point_info[1].u16PosY);
			}
			if(1 == revert_x_flag)
			{
		...
		case 1:
			...
			if(exchange_x_y_flag) 
			{
				swap(data->point_info[0].u16PosX, data->point_info[0].u16PosY);
			}
			if(1 == revert_x_flag)
			{
...
{	
	struct icn83xx_ts_data *icn83xx_ts= container_of(handler, struct icn83xx_ts_data, early_suspend);
...
static void ctp_load_param(void)
{
	SCREEN_MAX_X = env_get_u32("lcd_h_vd", 1024);
	SCREEN_MAX_Y = env_get_u32("lcd_v_vd", 768);
	
	revert_x_flag = env_get_u32("ctp_icn83xx_x_flag", 0);
	revert_y_flag = env_get_u32("ctp_icn83xx_y_flag", 0);
	exchange_x_y_flag = env_get_u32("ctp_icn83xx_x_y_flag", 0);
}
//END
...
static int icn83xx_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pr_info("touch panel gpio addr: = 0x%x", client->addr);
	this_client = client;
...
//B类	rockchip	     
	...
	client->irq=TS_INT;
	err = request_irq(client->irq, icn83xx_ts_interrupt,IRQF_TRIGGER_FALLING | IRQF_SHARED, client->name, icn83xx_ts);	
	if (err != 0)
	{
...
E_UPGRADE_ERR_TYPE  icn83xx_fw_update(char *fw)
{
	...
	_flash_info_("file_size=0x%x\n",file_size);
	if(icn83xx_goto_progmode() != 0)
	{
		if(icn83xx_check_progmod() < 0)
		{	
			strcpy(fw_status,"***err: icn83xx check progmod error***\n");
	...
	{	
		strcpy(fw_status,"***err: icn83xx_op1 error***\n");
...
//		icn83xx_memdump(temp_buf1, B_SIZE);
		...
		{	
			strcpy(fw_status,"***err: icn83xx_op2 error***\n");
...
//		icn83xx_memdump(temp_buf1, B_SIZE);		
	...
	for(j=0; j < num; j++)
	{
		...
		{	
			strcpy(fw_status,"***err: byte check error***\n");
	...
	{	
		strcpy(fw_status,"***err: upgrade checksum error***\n");
	...
	{	
		strcpy(fw_status,"***err: goto nomalmode error***\n");
	...
	strcpy(fw_status,"***OK: icn83xx FW ugrade successfuly***\n");
	_flash_info_("icn83xx FW ugrade successfuly\n");
	return R_OK;
}
...
int icn83xx_readVersion(void)
{
	...
	pr_info("icn83xx readVersion: 0x%x\n",CurVersion);
    return CurVersion;	
}


int icn83xx_readDevieID(void)
{
	int err = 0;
	char tmp[2];	
	int i;
	for(i=0; i<3; i++)
	{	
		err = icn83xx_i2c_rxdata(0x0f, tmp, 1);
		if(err >= 0)
			break;	
		msleep(5);
	}
	...
	pr_info("icn83xx readDevieID: 0x%x\n",tmp[0]);
    return tmp[0];	
}
...
static struct i2c_board_info i2c_info_dev =  {
	I2C_BOARD_INFO(CTP_NAME, ICN83XX_PROG_IIC_ADDR),
	.platform_data	= NULL,
};
