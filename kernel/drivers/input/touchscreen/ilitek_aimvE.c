...
{
	static int x = 0, y = 0;
	int i, len, ret, key,org_x=0, org_y=0,mult_tp_id,j;
	...
	if(i2c.protocol_ver & 0x200){
		...
		if(len== 0 && point_down){
			...
			//avoid report point(0,0) in some special case
			input_event(input, EV_ABS, ABS_MT_POSITION_X, x+1);
			input_event(input, EV_ABS, ABS_MT_POSITION_Y, y+1);
			input_report_key(input, BTN_TOUCH,  0);
			input_event(input, EV_ABS, ABS_MT_TOUCH_MAJOR, 0);
...
*/
static int 
__ilitek_i2c_register_device(
	void)
{
	int ret = 0;// = i2c_add_driver(&ilitek_i2c_driver);

	if(!ctp_supported("ilitek")) {
		return -ENODEV;
	}
...
// elf
static int 
ilitek_i2c_register_device(void)
{
	int ret = __ilitek_i2c_register_device();

	if (ret) {
		if (i2c.client)
			i2c_unregister_device(i2c.client);
	...
	void)
{
	...
	use_interupt=env_get_u32("ctp_ilitek_use_interupt", 0);
	printk("get ctp_ilitek_use_interupt=%d\n",use_interupt);
