...
static int max_brightness = BL_STEP;
static int capacity_cof1;
static int capacity_cof2;
static int rk29_bl_update_status(struct backlight_device *bl)
{
	...
	brightness *= (rk29_bl_info->max_brightness - rk29_bl_info->min_brightness);
	brightness /= BL_STEP;
...
static void handle_capacity(void * param, int status, int capacity)
{
	max_brightness = ((capacity * capacity_cof1) >> 16) + capacity_cof2;
	if(max_brightness < rk29_bl->props.brightness) {
		rk29_bl_update_status(rk29_bl);
	}
}

struct power_callback power_callback = {
	.mask = STATUS_CAPACITY,
	.param = 0,
...
{		
	...
	if(rk29_bl_info->max_brightness <= rk29_bl_info->min_brightness || rk29_bl_info->max_brightness > BL_STEP)
		rk29_bl_info->max_brightness = BL_STEP;

	if(rk29_bl_info->pwm_clock == 0)
		rk29_bl_info->pwm_clock = PWM_APB_PRE_DIV;
	...
	div_total = pwm_clk_rate / rk29_bl_info->pwm_clock;
	...
	capacity_cof1 = env_get_u32("lcd_bl_cof1", 0);
	if(capacity_cof1) {
		capacity_cof2 = env_get_u32("lcd_bl_cof2", 0);
		capacity_cof1 = capacity_cof1 * BL_STEP / 100;
		capacity_cof2 = capacity_cof2 * BL_STEP / 100;
		power_register_callback(&power_callback);
	}
