...
{
	unsigned int *src=(unsigned int *) bmp_logo->data;
	unsigned int *dst=(unsigned int *) info->screen_base;
	int w, h, bw, bh, cpd = 0;

	int sw = info->var.xres;
	int sh = info->var.yres;
	w = src[-1];
	
	bw = ((w & 0x7F) << 8) | ((w & 0xFF00) >> 8);
	bh = ((w & 0x7F0000) >> 8) | ((w & 0xFF000000) >> 24);
	cpd = bmp_logo->width * bmp_logo->height < bw * bh;

	printk("show logo s(%d %d), b(%d %d) compressed %d\n", sw, sh, bw, bh, cpd);
	w = sw;
	if(sw > bw) {
		dst += ((sw - bw) >> 1);
		w = bw;
