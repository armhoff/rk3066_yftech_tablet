...
{
	if(pin == RK30_PIN6_PB7) {
		pin = RK30_PIN4_PD7+1;
	}
	pin -= RK30_PIN0_PA0;
	if(pin < ARRAY_SIZE(rk30_muxs)) {
		rk30_muxs[pin].premode = rk30_muxs[pin].mode;
		rk30_muxs[pin].mode = mode;
		rk30_mux_set(&rk30_muxs[pin]);
	}
}
EXPORT_SYMBOL(yf_mux_api_set);

int yf_mux_api_get(unsigned pin)
{
	int ret = -1;
	if(pin == RK30_PIN6_PB7) {
		pin = RK30_PIN4_PD7+1;
	...
	pin -= RK30_PIN0_PA0;
	if(pin < ARRAY_SIZE(rk30_muxs)) {
		ret = readl(rk30_muxs[pin].mux_reg);
		ret = (ret >> rk30_muxs[pin].offset) &((1<<(rk30_muxs[pin].interleave))-1);
	}
	return ret;
}
EXPORT_SYMBOL(yf_mux_api_get);
